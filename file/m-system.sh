...
CEKEXPIRED () {
    ...
    Exp1=$(curl -sS https://raw.githubusercontent.com/sreyaeve/sshxrayv2/main/regip/ip | grep $MYIP | awk '{print $3}')
    if [[ $today < $Exp1 ]]; then
...
IZIN=$(curl -sS https://raw.githubusercontent.com/sreyaeve/sshxrayv2/main/regip/ip | awk '{print $4}' | grep $MYIP)
if [ $MYIP = $IZIN ]; then
